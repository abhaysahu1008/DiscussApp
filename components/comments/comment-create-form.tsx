        ...
        onClick={() => setOpen(!open)}>Reply</Button>
      {
        open && (
          <form action={action}>
            ...
            {formState.errors.formError && <div className='bg-red-200 border border-red-600 text-sm p-2 rounded mt-2'>{formState.errors.formError}</div>}
            <Button disabled={isPending} variant={'secondary'} size={'sm'} className='space-y-2'>
              {
                isPending ? (
                  <>
                    <Loader2 />
